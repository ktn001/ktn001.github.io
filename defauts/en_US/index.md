...
The **Faults** plugin detects faults by checking for consistency between a status and a measurement.

# Some examples:
{: .num}

- A light is on but no power is being used (defective bulb or faulty feedback signal).
- A light is off but consuming more than 1 watt (faulty feedback).
- The pump is running but there is no flow.

The plugin can also report faults when a measurement value deviates too far from a setpoint.
...
### Equipment
{: .num}
In addition to the standard configurations, the device has two settings that allow you to configure how **Auto Acknowledgment** works:
* **Auto Acknowledgment**
...
### Consistency checks
{: .num}
The `Monitoring` panel lets you manage equipment monitoring. The `Add Monitoring` button adds a consistency check to the equipment.

#### Consistency checks have several parameters:
{: .num}
![Configuring consistency monitoring](/images/defauts/config_surveillance.png)
* ***Name:*** Name of the monitoring task.
...
* ***Invert:*** Invert monitoring. The measurement must be greater than the threshold when the status is 0.
...
* ***Away:*** Monitoring is enabled when the ***status*** is 0 if this option is enabled.
* ***Log:*** Logging of information.
...
| Clearing a fault | ![](/images/faults/faults_cleared.png) | ![](/images/faults/faults_cleared_bw.png) | The fault was cleared by clicking on the icon (or through auto-clearance). The fault icon indicates that the fault is still present. |
...
| Cleared Alarms |![](/images/errors/errors_unacknowledged_alarm.png) | ![](/images/errors/errors_unacknowledged_alarm_bw.png) | The error icon indicates that there has been at least one alarm that has not been acknowledged. |
| Back to normal: | ![](/images/defaults/initial_defaults.png "Initial state") | ![](/images/defaults/initial_defaults_bw.png "Initial state") | The anomalies have been acknowledged and have disappeared. |
